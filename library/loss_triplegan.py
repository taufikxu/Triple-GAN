...
def loss_hinge_dis(netD, netG, netC, x_l, z_rand, label, x_u, x_u_d):
    ...
    label_real_for_d = torch.cat([label[:FLAGS.bs_l_for_d], l_d[:FLAGS.bs_u_for_d]], 0)
    ...
    return (loss_real + .5*loss_fake_g + .5*loss_fake_c).mean(), d_real.mean(), d_fake_g.mean(), d_fake_c.mean()


def loss_hinge_cla(netD, netC, x_u):
    probs = nn.Softmax(1)(netC(x_u))
    d_fake = netD(x_u)
