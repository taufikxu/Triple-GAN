...
@export
def tinyimagenet_cnn(pretrained=False, **kwargs):
    assert not pretrained
    model = CNN64(**kwargs)
    return model
...
@export
def stl10_cnn(pretrained=False, **kwargs):
    assert not pretrained
    model = CNN96(**kwargs)
    return model
...
@export
def stl10_shakeshake26(pretrained=False, **kwargs):
    assert not pretrained
    model = ResNet96(
        ShakeShakeBlock,
        layers=[4, 4, 4, 4],
...
class ResNet96(nn.Module):
    def __init__(
        ...
        self.inplanes = 16
        self.conv1 = nn.Conv2d(3, 16, kernel_size=3, stride=1, padding=1, bias=False)
        self.layer1 = self._make_layer(block, channels, groups, layers[0], stride=2)
        self.layer2 = self._make_layer(block, channels * 2, groups, layers[1], stride=2)
        self.layer3 = self._make_layer(block, channels * 4, groups, layers[2], stride=2)
        self.layer4 = self._make_layer(block, channels * 8, groups, layers[3], stride=2)
...
class CNN64(nn.Module):
    ...
    def __init__(self, num_classes=10):
        ...
        self.convpa = weight_norm(nn.Conv2d(3, 128, 3, padding=1))
        self.bnpa = nn.BatchNorm2d(128)
        self.mpp = nn.MaxPool2d(2, stride=2, padding=0)
        self.dropp = nn.Dropout(0.5)

        self.conv1a = weight_norm(nn.Conv2d(128, 128, 3, padding=1))
        self.bn1a = nn.BatchNorm2d(128)
        self.conv1b = weight_norm(nn.Conv2d(128, 128, 3, padding=1))
        self.bn1b = nn.BatchNorm2d(128)
        self.conv1c = weight_norm(nn.Conv2d(128, 128, 3, padding=1))
    ...
    def forward(self, x, debug=False):
        x = self.activation(self.bnpa(self.convpa(x)))
        x = self.mpp(x)
        x = self.dropp(x)
